...
function Navigo(appRoute, options) {
  ...
  function generate(name, data, options) {
    ...
    if (route) {
      ...
      if (data) {
        for (var key in data) {
          result = result.replace(":" + key, data[key]); // If regex group replacement is enabled, search
          // for named-regex parameter groups

          if ((options == null ? void 0 : options.replaceRegexGroups) === true) {
            var regexEscapedKey = key.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"); // Note that this won't handle regex groups that contain sub-groups
            // e.g. /(?<groupName>(:?example)?)

            var keyRegex = new RegExp("\\(\\?\\<" + regexEscapedKey + "\\>[^\\(^\\)]+\\)", "g");
